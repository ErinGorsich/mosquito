...
# Vector population parameters for population dynamics
A1 = 10
A2 = 20

# Rate mosquitoes move from incubating to infectious = 1 / EIP 
v.v <- 365 * 3 / 14  # 3 is for gamma distriputed EIP
...
params.two <- params <- list(
    # host parameters
    death.h = death.h, birth.h = birth.h, alpha.h = alpha.h, gamma.h = gamma.h, 
    u.h = u.h, Ho = Ho,
    # vector parameters
    A1 = A1, A2 = A2, D = D, d0 = d0, v.v = v.v, phi.v = phi.v,
    # transmission
    rho1.hv = rho1.hv, rho1.vh = rho1.vh, 
    rho2.hv = rho2.hv, rho2.vh = rho2.vh, bite = bite)

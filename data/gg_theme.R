...
gg_theme <- theme_bw() +
    ...
    theme(axis.title.x=element_text(margin=margin(8,0,0,0,"mm"), face="plain")) +
    theme(axis.title.y=element_text(margin=margin(0,8,0,0,"mm"), face="plain")) +
    theme(legend.key = element_blank()) +
    theme(legend.key.width = unit(7.5,"mm")) +
    theme(legend.key.height = unit(7.5,"mm")) +
    theme(legend.position = "none") +
    theme(legend.title = element_text(face="bold", size=14, margin=margin(0,0,0,0,"mm"))) +
    ...
    theme(title = element_text(face="bold", margin=margin(0, 0, 8, 0, "mm")))
